use crate::{
    aggregation::{
        srs, 
        srs::PreparedProverSRS, 
        transcript::{new_merlin_transcript, Transcript}},
        aggregation::{
            prover::aggregate_proofs,
            verifier::verify_aggregate_proof,
        },
...
fn legogroth16_aggregation() {
    ...
    let aggregate_proof_ =
        aggregate_proofs(prover_srs.clone(), &mut prover_transcript, &proofs)
    ...
    prover_transcript.append(b"public-inputs", &all_inputs);
    let aggregate_proof =
        aggregate_proofs(prepared_srs.clone(), &mut prover_transcript, &proofs)
            .expect("error in aggregation");
    println!(
