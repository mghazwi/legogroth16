...
mod test {
    ...
    use legogro16::link::{PESubspaceSnark, SparseMatrix, SubspaceSnark, PP};
    ...
    fn test_same_value_different_bases() {
        ...
        let y: Vec<G1Affine> = vec![
            bases1[0].into_group().mul(x[0]) + bases1[1].mul(x[2]),
            bases2[0].into_group().mul(x[1]) + bases2[1].mul(x[2]),
        ]
        .into_iter()
        .map(|p| p.into_affine())
        .collect::<Vec<_>>();
